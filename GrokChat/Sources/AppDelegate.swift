...
class SilentWebView: WKWebView {
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        // For Command shortcuts, let the menu system handle them normally
        if flags.contains(.command) {
            return super.performKeyEquivalent(with: event)
        }

        // For regular typing keys, let super try to handle it first
        // If super returns true, the key was handled (e.g., by web content)
        if super.performKeyEquivalent(with: event) {
            return true
        }

        // If super didn't handle it, return true anyway to prevent beep
        // The web content will still receive the key via JavaScript events
        return true
    }

    // Backup: Prevent beep when no responder handles an action selector
    override func noResponder(for eventSelector: Selector) {
        // Silently ignore - don't call super which would beep
    }
}
